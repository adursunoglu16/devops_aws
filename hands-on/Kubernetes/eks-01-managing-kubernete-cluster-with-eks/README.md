...
4. Run the command
```bash
aws eks --region <us-east-1> update-kubeconfig --name <cluster_name> # .kube file create ediyor(/home/ec2-user/.kube/config) so we are able to use kubectl
``` 
...
# once worker nodes delete sonra cluster.
# load balancer ve asg sil.