...
- kubeadm ile install edecegiz kubernetes i. Kubernetes i install eymek icin degisik toollar var bu da onlardan biri.
...
sudo apt-get install -y kubectl kubeadm kubelet kubernetes-cni docker.io ---->>>cni container network interface
```
...
# yukaridaki init komutundan sonra ekrana asagidaki kodlar geliyor clear etmeden kullan
  mkdir -p $HOME/.kube
  sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
  sudo chown $(id -u):$(id -g) $HOME/.kube/config

# kubernetes network isini kendi halletmiyor senin yapman lazim calico olur flannel olur hangisini istersen.
...
```bash
kubectl get pods -n kube-system -o wide  ----->>> o is output
```
...
# change token below. To get token use ---->>>>> `sudo kubeadm token create --print-join-command` and use what you get with "sudo kubeadm join 172.31.30.151:6443 --token m30m8k.slccsv874yf4y8d6 --discovery-token-ca-cert-hash sha256:5e32826633c76f296189b444b878e9e26cc93000a44437794b1df5671bf5eaf8"
```bash
...
# burada 80:32276/TCP 32276 portu dis dunyaya acilirken publicip/32276
```text
...
- To delete a worker/slave node from the cluster, follow the below steps.
  ...
  ```bash
  kubectl get nodes
  kubectl drain kube20-worker-1 --ignore-daemonsets --delete-local-data   --->>scheduler a diyozki bu nodeda calisma var buna bir sey atama
  kubectl cordon kube20-worker-1   ------->>>>scheduler a tekrar hazir bu diyoruz
...
```
# boylece bir certificate key alirsin

```bash
sudo kubeadm token create --certificate-key<certificate key that you get from above command>--print-join-command
```
# cikan kubeadm join ...... --control-plane olan kisma kadar al git worker node a yapistir. Eger control-plane kismini dalirsan bu sefer master node create edersin.
