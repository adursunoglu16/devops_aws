...
metadata:
  name: nginx-pod
  labels:                    # label icin istedigin isimde key pair yazabilirsin hello : world for example
    app: nginx
spec:
  containers:
  - name: mynginx
...
```bash
kubectl create -f mypod.yaml  ------->>> f means file
```
...
```bash
kubectl describe pods/nginx-pod or kubectl describe pods nginx-pod
```
...
spec:
  ...
  selector:
    matchLabels:
      app: nginx     # bu pod un label ile ayni olmali
  template:
    metadata:
      labels:
        app: nginx
...
```bash
kubectl delete replicaset <replicaset-name>  or kubectl delete -f myreplicaset.yaml
```
...
```bash
kubectl logs <pod-name>    # and docker logs containername for docker as you remember
```
...
```bash
kubectl delete deploy <deployment-name>    # or kubectl delete -f mydeployment.yaml
```
...
```bash
kubectl get deploy,rs,po -l app=container-info   # l means label
```
...
```bash
kubectl set image deploy clarus-deploy container-info=clarusway/container-info:2.0 --record=true # boylece yeni images ile olusturuldu eski versiyon 1.0 gitti 2.0 oldu
```
...
```bash
kubectl rollout history deploy clarus-deploy --revision=1  # cunku revision 1 revision 4 oldu geri donunce ona 1 gitti 4 oldu.
```
