...
```
# yani biz simdi bir volume olusturduk cluster havuzumuzda ve orada duruyor asagida onu request etcez PVC ile.
...
spec:
  ...
  resources:
    requests:
      storage: 5Gi     # yukarida 5gi olustrmustuk ornegin burada 6 gi isteseydik gelmicekti ama 4 isteseydik yukaridaki 5 gi gelcekti.
```
# 5 gi colume rquest ediyoruz PVC ile, yukarida zaten olusturmuk. 
  ...
  volumes:
    - name: clarus-pv-storage
      persistentVolumeClaim:
        claimName: clarus-pv-claim # az onceki pvc yi pod a bagliyoruz peki podda nereye o asagidaki aciklamada
  containers:
    - name: clarus-pv-container
      ...
      ports:
        - containerPort: 80
          name: "http-server"
      volumeMounts:
        - mountPath: "/usr/share/nginx/html"    # container icindeki bu klasoru clarus-pv-storage isimli
          name: clarus-pv-storage               # volume e bagladik
```
# volumude attach ediyoruz dikkat ettiysen.
# Dockerda bunu nasil yapardik imperative yontemle
---->>>  docker run --name clarus-pv-container -p 80:80 -v clarus-pv-storage:/usr/share/nginx/html image ......
