...
```bash
sudo hostnamectl set-hostname <node-name-manager-or-worker>    # after that type bash and push enter
```
...
```bash
docker swarm init    # we have just one ip for this instance so used this one
# or
docker swarm init --advertise-addr <Private IPs>
```
- In order to leave from docker swarm
```bash
