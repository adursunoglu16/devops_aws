...
```bash
docker run -it alpine ash    # ash, apline icin shell
```
...
```bash
docker volume inspect cw-vol     # volume un nerede oldugunu gosteriyor
```
...
```bash
docker run -it --name clarus -v cw-vol:/cw alpine ash or docker container run --name first -it --volume cw-vol:/test ubuntu bash
```
...
docker container prune      # volumeleri silmek icin once containerlari silmelisin cunku mounted-dependent
docker volume prune        
...
or---->>>>> docker volume create empty-vol && docker volume create full-vol
```
