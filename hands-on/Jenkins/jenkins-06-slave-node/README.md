...
```bash
sudo su - jenkins  # whoami ile bak, giremedigini goreceksin asagidakileri yap
```

- As you can see you cant connect 
```bash
cat /etc/passwd         # userlarin oldugu sayfada goreceksin jenkins icin false diyor so asagidakileri yap
...
```bash
ssh-keygen   # creates public and private rsa keys for both windows macos linux
```
